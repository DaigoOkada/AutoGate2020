...
path <- "/media/dokada/HD-LBVU3/MDS/test1/" #the path for output file
dir.create(path)
library(flowCore)
data_path <- "/media/dokada/HD-LBVU3/MDS/data/" #FCS files are put in this directory
markers <- c("PE-Cy7-A","PerCP-Cy5-5-A","APC-A","FITC-A","Pacific Blue-A", "PE-A")
genes <- c("CD19","IgM","IgD","CD21","CD27","CD138")
...
#the fnction calculating cutoff for all marker with EM
...
#Calculating cutoff for FCS files
set.seed(1000)
...
for(i in 1:length(files)){
  ...
  expr <- fcs@exprs[,markers]
  colnames(expr) <- genes
  result <- cell.percent1031(expr=expr,mu.ini.mat=mu.ini.mat.t,sigma.ini.mat=sigma.ini.mat.t,lambda.ini.mat=lambda.ini.mat.t,file_name=paste0(file1,".png"))
  kekka.cutoff <- rbind(kekka.cutoff,result)
  cat(i,"\n")
}
colnames(kekka.cutoff) <- genes
write.csv(kekka.cutoff,file=paste0(path,"kcb_tmp.csv"),quote=F)  #temporary cutoff matrix


###STEP3:Correction of cutoff value by fitting normal distribution to the distribution of cutoff value
library(MASS)
genes <- c("CD19","IgM","IgD","CD21","CD27","CD138")
markers <- c("PE-Cy7-A","PerCP-Cy5-5-A","APC-A","FITC-A","Pacific Blue-A", "PE-A")
...
kekka.cutoff <- read.csv(paste0(path,"kcb_tmp.csv"),header=T)
kc　<- kekka.cutoff[,genes]
...
write.csv(outlier.number,file=paste0(path,"outlier.number.csv"),quote=F) #the number of outlier
write.csv(kct_comp,file=paste0(path,"kcb_comp.csv"),quote=F) #the cutoff matrix after correction
...
cell.percent3b <- function(expr,cutoff.vec){
  ...
  pn.mat <- sapply(1:6,function(i){ifelse(expr[,i] < cutoff.vec[i],0,1)})  #positive:１,negative:0
  ...
  for(c1 in c(0,1)){
      for(c3 in c(0,1)){
        for(c4 in c(0,1)){
          for(c5 in c(0,1)){
              tmp <- intersect(which(pn.mat[,1]==c1),which(pn.mat[,3]==c3)) #Note:IgM and CD138 are removed
              tmp <- intersect(tmp,which(pn.mat[,4]==c4))
              tmp <- intersect(tmp,which(pn.mat[,5]==c5))
              clcnt <- c(clcnt,length(tmp))
              clcnt.name <- c(clcnt.name,paste0(c1,c3,c4,c5))
          }
        }
      }
  }
...
library(flowCore)
files <- sort(list.files(data_path))
kc.comp <- read.csv("kcb_comp.csv",header=1,row.names=1)
...
write.csv(kekka.proportion,file=paste0(path,"kpb_new.csv"),quote=F,row.names=F) # B cell proportion matrix
